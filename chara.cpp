...
nlohmann::json *loadCharFile(const std::string &charName, int version, const std::string &jsonName)
{
    ...
    while (versionSlot >= 0) {
        ...
        if (charFileExists(charPath, charFileName)) {
            foundFile = true;
            break;
        }
        versionSlot--;
    ...
    if (mapCharFileLoader.find(charFileName) == mapCharFileLoader.end()) {
        std::string fullPath = charPath + charFileName;
        if (std::filesystem::exists(fullPath)) {
            mapCharFileLoader[charFileName] = parse_json_file(fullPath);
        } else {
...
void loadChargesFChar(nlohmann::json* pCharJson, CharacterData* pRet)
{
    ...
    for (int i = 0; i < (*pCharJson)["dataListIdTbl"].size(); i++) {
        if ((*pCharJson)["dataListIdTbl"][i] == (int)DataListId::ChargeParamSub) {
            chargeIdx = i;
            break;
        }
    }
    ...
    nlohmann::json &chargeJson = (*pCharJson)["dataListTbl"][chargeIdx];
    ...
    for (int i = 0; i < chargeJson["idTbl"].size(); i++) {
        Charge charge;
        int chargeID = chargeJson["idTbl"][i];
        nlohmann::json &chargeParam = chargeParamJson[i];
        charge.id = chargeID;
        charge.okKeyFlags = chargeParam["Parameters"]["OKKey"];
        charge.okCondFlags = chargeParam["Parameters"]["OKPara"]["_LeverCond"].get<int>() | (chargeParam["Parameters"]["OKPara"]["_KeyCond"].get<int>() << 8);
        charge.chargeFrames = chargeParam["OK_Frame"];
        charge.keepFrames = chargeParam["KEEP_Frame"];
...
void loadCommandsFChar(nlohmann::json* pCharJson, CharacterData* pRet)
{
    ...
    for (int i = 0; i < (*pCharJson)["dataListIdTbl"].size(); i++) {
        if ((*pCharJson)["dataListIdTbl"][i] == (int)DataListId::CommandParamSub) {
            commandIdx = i;
            break;
        }
    }
    ...
    nlohmann::json &commandJson = (*pCharJson)["dataListTbl"][commandIdx];
    ...
    for (int i = 0; i < commandJson["idTbl"].size(); i++) {
        ...
        Command command;
        command.id = commandID;
        for (auto& cmnCommand : pRet->commands)
        {
            if (cmnCommand.id == command.id) {
                command = cmnCommand;
                break;
            }
        }
        nlohmann::json &commandGroup = commandParamJson[i]["Commands"]["items"];
        command.variants.reserve(commandGroup.size());

        for (int i = 0; i < commandGroup.size(); i++) {
            auto& variant = commandGroup[i];
            int inputNum = variant["CommandSub"]["items"].size();
            nlohmann::json *pInputs = &variant["CommandSub"]["items"];
            ...
            for (int i = 0; i < inputNum; i++) {
                ...
                commandVariant.totalMaxFrames += input.numFrames;
                ...
                input.okCondFlags |= (*pInputNorm)["STRUCT_OKPara__KeyCond"].get<int>() << 8;
                input.ngKeyFlags = (*pInputNorm)["NGKey"];
                input.ngCondFlags = (*pInputNorm)["STRUCT_NGPara__LeverCond"];
                input.ngCondFlags |= (*pInputNorm)["STRUCT_NGPara__KeyCond"].get<int>() << 8;
                input.failKeyFlags = (*pInputNorm)["XXKey"];
                input.failCondFlags = (*pInputNorm)["STRUCT_XXPara__LeverCond"];
                input.failCondFlags |= (*pInputNorm)["STRUCT_XXPara__KeyCond"].get<int>() << 8;
...
void loadTriggersFChar(nlohmann::json* pCharJson, CharacterData* pRet)
{
    if (!pCharJson || !pCharJson->contains("dataListIdTbl") || !pCharJson->contains("dataListIdTbl")) {
        return;
    }
    ...
    for (int i = 0; i < (*pCharJson)["dataListIdTbl"].size(); i++) {
        if ((*pCharJson)["dataListIdTbl"][i] == (int)DataListId::TriggerTrigger) {
            triggerIdx = i;
            break;
        }
    }
    ...
    nlohmann::json &triggerJson = (*pCharJson)["dataListTbl"][triggerIdx];
    ...
    for (int i = 0; i < triggerJson["idTbl"].size(); i++) {
        ...
        for (auto& cmnTrigger : pRet->triggers)
        {
            if (cmnTrigger.id == trigger.id) {
                trigger = cmnTrigger;
                break;
            }
        }
        ...
        {
            trigger.validStyles = (*pCondition)["_ValidStyle"][0].get<int>();
        }

        nlohmann::json *pNorm = &triggerData["Normal"];
        trigger.okKeyFlags = (*pNorm)["Key"];
        auto leverCond = (*pNorm)["Lever"]["Cond"].get<int>();
        auto buttonCond = (*pNorm)["Button"]["Cond"].get<int>();
        auto leverState = (*pNorm)["Lever"]["State"].get<int>();
        auto buttonState = (*pNorm)["Button"]["State"].get<int>();
        trigger.okCondFlags = leverCond & 7 | (32 * (buttonCond & 7 | (32 * (leverState & 0xF | (16 * (buttonState & 0xF))))));
        ...
        {
            trigger.precedingTime = (triggerData["Tags"]["Flags"].get<int>() >> 29) & 2 | 4;
        }
        ...
        if (triggerData["Tags"]["Flags"].get<uint64_t>() & 0x20000000000000ull)
        {
            optionFlag |= 8;
        }
        ...
        trigger.condParamID = triggerData["Condition"]["UParamType"].get<int>() - 1;
...
void loadTriggerGroups(nlohmann::json* pTriggerGroupsJson, CharacterData* pRet)
{
    ...
    for (auto& [groupIDStr, group] : pTriggerGroupsJson->items()) {
        ...
        bool bIsCommon = false;
        for (auto& cmnTriggerGroup : pRet->triggerGroups)
        {
            if (cmnTriggerGroup.id == triggerGroup.id) {
                cmnTriggerGroup = triggerGroup;
                bIsCommon = true;
                break;
            }
        }
...
void loadTriggerGroupsFChar(nlohmann::json* pCharJson, CharacterData* pRet)
{
    ...
    for (int i = 0; i < (*pCharJson)["dataListIdTbl"].size(); i++) {
        if ((*pCharJson)["dataListIdTbl"][i] == (int)DataListId::TriggerGroup) {
            triggerGroupIdx = i;
            break;
        }
    }
    ...
    nlohmann::json &triggerGroupJson = (*pCharJson)["dataListTbl"][triggerGroupIdx];
    ...
    for (int i = 0; i < triggerGroupJson["idTbl"].size(); i++) {
        ...
        {
            if (cmnTriggerGroup.id == triggerGroup.id) {
                triggerGroup = cmnTriggerGroup;
                break;
            }
        }

        for (int i = 0; i < group["SelectTriggers"].size(); i++) {
            auto selectTrigger = group["SelectTriggers"][i];
...
{
    if (!pCharJson || !pCharJson->contains("dataListIdTbl") || !pCharJson->contains("dataListIdTbl") || !pOutputVector) {
        return;
    }
...
void loadHurtBoxKeysFChar(nlohmann::json* pHurtBoxJson, std::vector<HurtBoxKey>* pOutputVector, std::map<std::pair<int, int>, Rect*>& rectsByIDs, std::map<int, AtemiData*>& atemiByID)
{
    if (!pHurtBoxJson) {
        return;
    }
    ...
    for (int i = 0; i < (*pHurtBoxJson)["dataTbl"].size(); i++) {
        ...
        HurtBoxKey newKey;
        newKey.startFrame = key["startFrame"];
        newKey.endFrame = key["endFrame"];
        newKey.condition = hurtBox["Condition"];
        newKey.offsetX = Fixed(hurtBox["STRUCT_RootOffset_X"].get<int>());
        newKey.offsetY = Fixed(hurtBox["STRUCT_RootOffset_Y"].get<int>());
        newKey.isArmor = hurtBox["CollisionType"] == 0x1;
        int armorID = hurtBox["AtemiDataListIndex"];
        auto atemiIt = atemiByID.find(armorID);
        if (atemiIt != atemiByID.end()) {
            newKey.pAtemiData = atemiIt->second;
        }
        newKey.isAtemi = hurtBox["CollisionType"] == 0x2;
        newKey.immunity = hurtBox["Immune"];
        newKey.flags = hurtBox["TypeFlag"];
...
void loadPushBoxKeysFChar(nlohmann::json* pPushBoxJson, std::vector<PushBoxKey>* pOutputVector, std::map<std::pair<int, int>, Rect*>& rectsByIDs)
{
    if (!pPushBoxJson) {
        return;
    }
...
void loadHitBoxKeysFChar(nlohmann::json* pHitBoxJson, std::vector<HitBoxKey>* pOutputVector, std::map<std::pair<int, int>, Rect*>& rectsByIDs, bool isOther, std::map<int, HitData*>& hitByID)
{
    if (!pHitBoxJson) {
        return;
    }
    ...
    for (int i = 0; i < (*pHitBoxJson)["dataTbl"].size(); i++) {
        ...
        if (key["_ValidStyle"].size()) 
        {
            newKey.hasValidStyle = true;
            newKey.validStyle = key["_ValidStyle"][0].get<int>();
        }
        ...
        int hitID = hitBox["HitID"];
        bool hasHitID = hitBox["CollisionType"].get<int>() < 3 || (isOther ? (1 << (hitBox["CollisionType"].get<int>() & 0x1F)) & 0x380 : (1 << (hitBox["CollisionType"].get<int>() & 0x1F)) & 0x1000);
        if (hitID < 0) {
            hitID = 15;
        }
        // todo maybe need to use erase id?
...
void loadUniqueBoxKeysFChar(nlohmann::json* pHitBoxJson, std::vector<UniqueBoxKey>* pOutputVector, std::map<std::pair<int, int>, Rect*>& rectsByIDs)
{
    if (!pHitBoxJson) {
        return;
    }
    ...
    for (int i = 0; i < (*pHitBoxJson)["dataTbl"].size(); i++) {
        ...
        newKey.ops.reserve(hitBox["Datas"]["items"].size());
        for (int i = 0; i < hitBox["Datas"]["items"].size(); i++) {
            auto& dataOp = hitBox["Datas"]["items"][i];
            UniqueBoxOp newOp;
            newOp.op = dataOp["OpeType"];
            newOp.opParam0 = dataOp["Param00"];
            newOp.opParam1 = dataOp["Param01"];
...
void loadSteerKeysFChar(nlohmann::json* pSteerJson, std::vector<SteerKey>* pOutputVector, bool isDrive)
{
    if (!pSteerJson) {
        return;
    }
...
void loadPlaceKeysFChar(nlohmann::json* pPlaceJson, std::vector<PlaceKey>* pOutputVector, bool isBG)
{
    if (!pPlaceJson) {
        return;
    }

    for (int i = 0; i < (*pPlaceJson)["dataTbl"].size(); i++) {
        auto& key = (*pPlaceJson)["dataTbl"][i];
        auto& placeKey = (*pPlaceJson)["obj"][i];
        if (!key.contains("startFrame")) {
            continue;
        }
        PlaceKey newKey;
        ...
        newKey.endFrame = key["endFrame"];
        newKey.optionFlag = placeKey.value("OptionFlag", 0);
        newKey.ratio = Fixed((double)placeKey["Ratio"]["v"] / 65536.0);
        newKey.axis = placeKey.value("Axis", 0);
        newKey.bgOnly = isBG;

        if (placeKey.contains("PosList")) {
            newKey.posList.reserve(placeKey["PosList"].size());
            for (int i = 0; i < placeKey["PosList"].size(); i++) {
                PlaceKeyPos pos;
                pos.frame = i;
                pos.offset = Fixed((double)placeKey["PosList"][i]["v"] / 65536.0);
                newKey.posList.push_back(pos);
            }
        }
...
void loadSwitchKeysFChar(nlohmann::json* pSwitchJson, std::vector<SwitchKey>* pOutputVector)
{
    if (!pSwitchJson) {
        return;
    }

    for (int i = 0; i < (*pSwitchJson)["dataTbl"].size(); i++) {
        auto& key = (*pSwitchJson)["dataTbl"][i];
        auto& switchKey = (*pSwitchJson)["obj"][i];
        if (!key.contains("startFrame")) {
            continue;
        }
        SwitchKey newKey;
        ...
        newKey.systemFlag = switchKey["SystemFlag"];
        newKey.operationFlag = switchKey["OperationFlag"];
        newKey.validStyle = switchKey["_ValidStyle"].size() ? switchKey["_ValidStyle"][0].get<int>() : 0;
...
void loadEventKeysFChar(nlohmann::json* pEventJson, std::vector<EventKey>* pOutputVector)
{
    if (!pEventJson) {
        return;
    }

    for (int i = 0; i < (*pEventJson)["dataTbl"].size(); i++) {
        auto& key = (*pEventJson)["dataTbl"][i];
        auto& eventKey = (*pEventJson)["obj"][i];
        if (!key.contains("startFrame")) {
            continue;
        }
        EventKey newKey;
        newKey.startFrame = key["startFrame"];
        newKey.endFrame = key["endFrame"];
        newKey.validStyle = eventKey["_ValidStyle"].size() ? eventKey["_ValidStyle"][0].get<int>() : 0;
        newKey.type = eventKey["Type"];
        newKey.id = eventKey["ID"];
        newKey.param01 = eventKey["Param01"];
        newKey.param02 = eventKey["Param02"];
...
void loadWorldKeysFChar(nlohmann::json* pWorldJson, std::vector<WorldKey>* pOutputVector)
{
    if (!pWorldJson) {
        return;
    }

    for (int i = 0; i < (*pWorldJson)["dataTbl"].size(); i++) {
        auto& key = (*pWorldJson)["dataTbl"][i];
        auto& worldKey = (*pWorldJson)["obj"][i];
        if (!key.contains("startFrame")) {
            continue;
        }
        WorldKey newKey;
...
void loadLockKeysFChar(nlohmann::json* pLockJson, std::vector<LockKey>* pOutputVector, std::map<int, HitData*>& hitByID)
{
    if (!pLockJson) {
        return;
    }

    for (int i = 0; i < (*pLockJson)["dataTbl"].size(); i++) {
        auto& key = (*pLockJson)["dataTbl"][i];
        auto& lockKey = (*pLockJson)["obj"][i];
        if (!key.contains("startFrame")) {
            continue;
        }
        LockKey newKey;
...
void loadBranchKeysFChar(nlohmann::json* pBranchJson, std::vector<BranchKey>* pOutputVector)
{
    if (!pBranchJson) {
        return;
    }

    for (int i = 0; i < (*pBranchJson)["dataTbl"].size(); i++) {
        auto& key = (*pBranchJson)["dataTbl"][i];
        auto& branchKey = (*pBranchJson)["obj"][i];
        if (!key.contains("startFrame")) {
            continue;
        }
        BranchKey newKey;
        ...
        newKey.branchAction = branchKey["Action"];
        newKey.branchFrame = branchKey["ActionFrame"];
        newKey.keepFrame = branchKey["Attr"].get<int>() & 0x100 || branchKey["Attr"].get<int>() & 0x2;
        newKey.keepPlace = branchKey["Attr"].get<int>() & 0x20;
        newKey.typeName = branchTypes[newKey.type];
...
void loadShotKeysFChar(nlohmann::json* pShotJson, std::vector<ShotKey>* pOutputVector)
{
    if (!pShotJson) {
        return;
    }

    for (int i = 0; i < (*pShotJson)["dataTbl"].size(); i++) {
        auto& key = (*pShotJson)["dataTbl"][i];
        auto& shotKey = (*pShotJson)["obj"][i];
        if (!key.contains("startFrame")) {
            continue;
        }
        ShotKey newKey;
        newKey.startFrame = key["startFrame"];
        newKey.endFrame = key["endFrame"];
        newKey.validStyle = shotKey["_ValidStyle"].size() ? shotKey["_ValidStyle"][0].get<int>() : 0;
        newKey.operation = shotKey["Operation"];
        newKey.flags = shotKey["SpawnFlag"];
        newKey.posOffsetX = Fixed((double)shotKey["PosOffset"]["x"]["v"] / 65536.0);
        newKey.posOffsetY = Fixed((double)shotKey["PosOffset"]["y"]["v"] / 65536.0);
        newKey.actionId = shotKey["ActionId"];
        newKey.styleIdx = shotKey["StyleIdx"];
...
void loadTriggerKeysFChar(nlohmann::json* pTriggerJson, std::vector<TriggerKey>* pOutputVector, std::map<int, TriggerGroup*>& triggerGroupByID)
{
    if (!pTriggerJson) {
        return;
    }

    for (int i = 0; i < (*pTriggerJson)["dataTbl"].size(); i++) {
        auto& key = (*pTriggerJson)["dataTbl"][i];
        auto& triggerKey = (*pTriggerJson)["obj"][i];
        if (!key.contains("startFrame")) {
            continue;
        }
        TriggerKey newKey;
        newKey.startFrame = key["startFrame"];
        newKey.endFrame = key["endFrame"];
        newKey.validStyle = triggerKey["_ValidStyle"].size() ? triggerKey["_ValidStyle"][0].get<int>() : 0;
        newKey.other = triggerKey["ConditionFlag"].get<int>() & 0x20040;
        newKey.condition = triggerKey["ConditionFlag"].get<int>() & 0x3C0F;
        newKey.state = triggerKey["ConditionFlag"].get<int>() & 0x3C03B0;
...
void loadStatusKeysFChar(nlohmann::json* pStatusJson, std::vector<StatusKey>* pOutputVector)
{
    if (!pStatusJson) {
        return;
    }

    for (int i = 0; i < (*pStatusJson)["dataTbl"].size(); i++) {
        auto& key = (*pStatusJson)["dataTbl"][i];
        auto& statusKey = (*pStatusJson)["obj"][i];
        if (!key.contains("startFrame")) {
            continue;
        }
        StatusKey newKey;
...
void loadHitsFChar(nlohmann::json* pCharJson, std::vector<HitData>* pOutputVector)
{
    ...
    for (int i = 0; i < (*pCharJson)["dataListIdTbl"].size(); i++) {
        if ((*pCharJson)["dataListIdTbl"][i] == (int)DataListId::AttackDataParams) {
            hitIdx = i;
            break;
        }
    }
    ...
    nlohmann::json &hitJson = (*pCharJson)["dataListTbl"][hitIdx];
    ...
    for (int i = 0; i < hitJson["idTbl"].size(); i++) {
        ...
        nlohmann::json &hitData = hitParamJson[i]["Root"];
        ...
        auto loadHitEntryFChar = [](nlohmann::json &hitData, HitData& newHit, bool bCommon) {
            ...
            for (int i = 0; i < gaugeExs.size(); i++)
            {
                auto& gaugeEx = gaugeExs[i];
                int idx = gaugeEx["IndexID"];
                auto& slot = bCommon ? newHit.common[idx] : newHit.param[idx];
                slot.focusGainTarget = gaugeEx["FocusTarget0"];
            }
            ...
            for (int i = 0; i < timers.size(); i++)
            {
                ...
                auto& slot = bCommon ? newHit.common[idx] : newHit.param[idx];
                slot.hitStopOwner = timer["HitStopOwner"];
                slot.hitStopTarget = timer["HitStopTarget"];
                slot.curveOwnID = timer["AttackOwnerID"];
                slot.curveTargetID = timer["AttackTargetID"];
                slot.downTime = timer["DownTime"];
            }
        ...
        loadHitEntryFChar(hitData, newHit, true);
        loadHitEntryFChar(hitData, newHit, false);
...
int countAtemisFChar(nlohmann::json* pCharJson)
{
    ...
    for (int i = 0; i < (*pCharJson)["dataListIdTbl"].size(); i++) {
        if ((*pCharJson)["dataListIdTbl"][i] == (int)DataListId::AtemiData) {
            atemiIdx = i;
            break;
        }
    }
    ...
    nlohmann::json &atemiJson = (*pCharJson)["dataListTbl"][atemiIdx];
...
void loadAtemisFChar(nlohmann::json* pCharJson, std::vector<AtemiData>* pOutputVector)
{
    ...
    pOutputVector->reserve(atemiJson["idTbl"].size());
...
void loadProjectileDatasFChar(nlohmann::json* pCharJson, std::map<int, ProjectileData>* pUniqueProjectiles)
{
    ...
    for (int i = 0; i < (*pCharJson)["dataListIdTbl"].size(); i++) {
        if ((*pCharJson)["dataListIdTbl"][i] == (int)DataListId::ProjectileData) {
            projIdx = i;
            break;
        }
    }
    ...
    nlohmann::json &projJson = (*pCharJson)["dataListTbl"][projIdx];
    ...
    for (int i = 0; i < projJson["idTbl"].size(); i++) {
        ...
        //     newProj.extraHitStop = 0;
        // }
        newProj.hitFlagToParent = projData["Attr0"].get<int>() & 8;
        newProj.hitStopToParent = projData["Attr0"].get<int>() & 4;
        ...
        newProj.hitStopToParent = projData["Attr0"].get<int>() & 0x80000000;
        ...
        newProj.noPush = projData["Attr0"].get<int>() & 0x40000;
        ...
        (*pUniqueProjectiles)[projJson["idTbl"][i]] = newProj;    
    }
}
...
void loadActionsFromMovesFChar(nlohmann::json* pCharJson, nlohmann::json* pNamesJson, CharacterData* pRet, std::map<std::pair<int, int>, Rect*>& rectsByIDs, std::map<int, AtemiData*>& atemiByID, std::map<int, HitData*>& hitByID, std::map<int, TriggerGroup*>& triggerGroupByID)
{
    if (!pCharJson || !pCharJson->contains("actionListTbl")) {
        return;
    }

    nlohmann::json &actionListTblJson = (*pCharJson)["actionListTbl"];
    ...
    for (int i = 0; i < actionListTblJson.size(); i++)
    {
        ...
        for (int j = 0; j < style["dataTbl"].size(); j++)
        {
            ...
            newAction.styleID = i;
            std::string actionIDStr = to_string_leading_zeroes(newAction.actionID, 4);
            if (pNamesJson->contains(actionIDStr))
                newAction.name = (*pNamesJson)[actionIDStr];
            else
            ...
            newAction.actionFlags = fab["Category"]["Flags"];
            newAction.actionFrameDuration = action["frames"];
            newAction.loopPoint = fab["State"]["EndStateParam"];
            newAction.loopCount = fab["State"]["LoopCount"];
            ...
            if (fab.contains("Inherit")) {
                ...
                if (pInherit->contains("Velocity")) {
                    newAction.inheritVelX = Fixed((double)(*pInherit)["Velocity"]["x"]["v"] / 65536.0);
                    newAction.inheritVelY = Fixed((double)(*pInherit)["Velocity"]["y"]["v"] / 65536.0);
                }
            }

            for (auto& key : action["dataTbl"])
            {
                std::string type = key["obj"][0]["$type"];
                if (type == "CharacterAsset.DamageCollisionKey")
                {
                    loadHurtBoxKeysFChar(&key, &newAction.hurtBoxKeys, rectsByIDs, atemiByID);
                }
                if (type == "CharacterAsset.PushCollisionKey")
                {
                    loadPushBoxKeysFChar(&key, &newAction.pushBoxKeys, rectsByIDs);
                }
                if (type == "CharacterAsset.OtherCollisionKey")
                {
                    loadHitBoxKeysFChar(&key, &newAction.hitBoxKeys, rectsByIDs, true, hitByID);
                }
                if (type == "CharacterAsset.AttackCollisionKey")
                {
                    loadHitBoxKeysFChar(&key, &newAction.hitBoxKeys, rectsByIDs, false, hitByID);
                }
                if (type == "CharacterAsset.UniqueCollisionKey") 
                {
                    newAction.uniqueBoxKeys.reserve(key["dataTbl"].size() - 1);
                    loadUniqueBoxKeysFChar(&key, &newAction.uniqueBoxKeys, rectsByIDs);
                }
                if (type == "CharacterAsset.SteerKey") 
                {
                    loadSteerKeysFChar(&key, &newAction.steerKeys, false);
                }
                if (type == "CharacterAsset.DriveSteerKey") 
                {
                    loadSteerKeysFChar(&key, &newAction.steerKeys, true);
                }
                if (type == "CharacterAsset.PlaceKey") {
                    loadPlaceKeysFChar(&key, &newAction.placeKeys, false);
                }
                if (type == "CharacterAsset.BGPlaceKey") {
                    loadPlaceKeysFChar(&key, &newAction.placeKeys, true);
                }
                if (type == "CharacterAsset.SwitchKey") {
                    loadSwitchKeysFChar(&key, &newAction.switchKeys);
                }
                if (type == "CharacterAsset.ExtSwitchKey") {
                    loadSwitchKeysFChar(&key, &newAction.switchKeys);
                }
                if (type == "CharacterAsset.EventKey") {
                    loadEventKeysFChar(&key, &newAction.eventKeys);
                }
                if (type == "CharacterAsset.WorldKey") {
                    loadWorldKeysFChar(&key, &newAction.worldKeys);
                }
                if (type == "CharacterAsset.LockKey") {
                    loadLockKeysFChar(&key, &newAction.lockKeys, hitByID);
                }
                if (type == "CharacterAsset.BranchKey") {
                    loadBranchKeysFChar(&key, &newAction.branchKeys);
                }
                if (type == "CharacterAsset.ShotKey") {
                    loadShotKeysFChar(&key, &newAction.shotKeys);
                }
                if (type == "CharacterAsset.TriggerKey") {
                    loadTriggerKeysFChar(&key, &newAction.triggerKeys, triggerGroupByID);
                }
                if (type == "CharacterAsset.StatusKey") {
                    loadStatusKeysFChar(&key, &newAction.statusKeys);
                }
            }
            bool bIsCommon = false;
            for (auto& cmnAction : pRet->actions) {
...
CharacterData *loadCharacter(std::string charName, int charVersion)
{
    ...
    if (pCharJson && pCommonJson) {
        ...
        loadCommandsFChar(pCommonJson, pRet);
        loadCommandsFChar(pCharJson, pRet);
        loadTriggersFChar(pCommonJson, pRet);
        ...
        for (auto& triggerGroupEntry : triggerGroup0->entries) {
            ...
            std::string actionString = actionID + " " + (*pNamesJson).value("actionID", "Unknown action");
        ...
        loadHitsFChar(pCharJson, &pRet->hits);

        for (auto& hit : pRet->hits) {
            pRet->hitByID[hit.id] = &hit;
        }

        loadActionsFromMovesFChar(pCommonJson, pNamesJson, pRet, pRet->rectsByIDs, pRet->atemiByID, pRet->hitByID, pRet->triggerGroupByID);
        for (auto& action : pRet->actions) {
            action.common = true;
        }
        loadActionsFromMovesFChar(pCharJson, pNamesJson, pRet, pRet->rectsByIDs, pRet->atemiByID, pRet->hitByID, pRet->triggerGroupByID);
...
bool cookCharacter(CharacterData* pData, const std::string& path)
{
    ...
    for (auto& proj : pData->projectileDatas) {
        ...
        writeI32(f, proj.hitStopOverride);
    }
    ...
    for (auto& atemi : pData->atemis) {
        ...
        writeI32(f, atemi.targetStopProj);
        writeI32(f, atemi.ownerStopProj);
        writeI32(f, atemi.targetStopAdd);
        writeI32(f, atemi.ownerStopAdd);
        writeI32(f, atemi.targetStopAddProj);
        writeI32(f, atemi.ownerStopAddProj);
...
CharacterData* loadCookedCharacter(const std::string& path, int charVersion)
{
    ...
    for (auto& proj : pRet->projectileDatas) {
        ...
        proj.hitStopOverride = readI32(f);
    }
    ...
    for (auto& atemi : pRet->atemis) {
        ...
        atemi.targetStopProj = readI32(f);
        atemi.ownerStopProj = readI32(f);
        atemi.targetStopAdd = readI32(f);
        atemi.ownerStopAdd = readI32(f);
        atemi.targetStopAddProj = readI32(f);
        atemi.ownerStopAddProj = readI32(f);
